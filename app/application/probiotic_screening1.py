...
class ProbioticScreening(object):
    def __init__(self, sampleid, biom_path, out_dir):
        self.sampleid = sampleid
        self.biom_path = biom_path
        self.out_dir = out_dir

    def getshai(self, sampleid, shstr, outpath, current_path, biom_path):  # shaixuanban
        print("getshai is running")
        content = self.select_sql(sampleid)
        rejson, ddict = self.getone(biom_path, shstr, outpath, current_path)
        self.render_report(ddict, content, outpath, sampleid)
    ...
    def getone(self, file0, shstr, outpath, current_path):  # file0:SGC0200919      /share/douhy/shuguo/SGC0200919.biom
        ...
        os.system(shstr)
        outfile = filename + '/bigfile.json'
        dfrefile = filename + '/dfre.csv'
        dfre = self.getfulltable(littlefilename, filename, current_path)
        # print dfre.head()
        dfre.to_csv(dfrefile)

        rejson, ddict = self.getjson(dfre)
    ...
    def getfulltable(self, littlefilename, filename, current_path):
    ...
    def getjson(self, dfre):
        ...
        for i in range(len(zuolist)):
            ddict[(self.chulikongge(zuolist[i]) + "_min")] = minlist[i]
            ddict[(self.chulikongge(zuolist[i]) + "_max")] = maxlist[i]
        for k in ddict:
            try:
    ...
    def getall(self, ff, shstr, outpath, current_path):
        for i in ff:
            shstra, filename = self.getone(i, shstr, outpath, current_path)
            # print shstra
            os.system(shstra)
            outfile = filename + '/result.json'
            dfrefile = filename + '/dfre.csv'
            dfre = self.getfulltable(i, filename, current_path)
            # print dfre.head()
            dfre.to_csv(dfrefile)
            rejson, ddict = self.getjson(dfre)
    ...
    def select_sql(self, sampleid):
        ...
        DB_CONNECT = 'mysql+pymysql://{}:{}@{}/{}?charset=utf8'.format(user, pwd, dbaddress, DBName)
        print(DB_CONNECT)
        ...
        sample = session.query(TubeInfor).filter_by(Tube_ID=sampleid).first()
        if sample:
    ...
    def render_report(self, ddict, content, outpath, sampleid):
        ...
        tpl.render(content)
        outfile = os.path.join(outpath, sampleid + f"/{sampleid}_PY_probiotics_report.docx")
        tpl.save(outfile)
    ...
    def run(self):
        ...
        alpha_diversity.py -i $workdir/otu_table_even10000.biom -m observed_species -o $workdir/otu_table_no_singletons.obs''' % (
            activate_path, qimme_path, current_path, current_path, current_path, current_path, current_path,
            current_path)

        # a,b = getone(options.infilename, shstr, options.outfile, current_path)
        self.getshai(self.sampleid, shstr, self.out_dir, current_path, self.biom_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--sampleid", help="sampleid", metavar="FILE", default="SA1901109")
    parser.add_argument("-b", "--biom_path", help="biom path", metavar="FILE")
    parser.add_argument("-o", "--out_dir",  help="output prefix, the default is .", metavar="FILE", default=".")
    args = parser.parse_args()
    # logger.debug(args.tsvfile, args.txtpath, args.outfna)
    ps = ProbioticScreening(args.sampleid, args.biom_path, args.out_dir)
    ps.run()
